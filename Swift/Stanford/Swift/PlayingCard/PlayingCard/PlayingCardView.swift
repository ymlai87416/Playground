...
class PlayingCardView: UIView {
    ...
    var faceCardScale: CGFloat = SizeRatio.faceCardImageSizeToBoundSize { didSet{
        setNeedsDisplay()
        }}
    ...
    override func layoutSubviews(){
        ...
        upperLeftCornerLabel.frame.origin =
    ...
    private func drawPips(){
        let pipsPerFowForRank = [[0],[1],[1,1],[1,1,1],[2,2],[2,1,2],[2,2,2],[2,1,2,2],[2,2,2,2],[2,2,1,2,2],[2,2,2,2,2]]
        
        func createPipString(thatFits pipRect: CGRect) -> NSAttributedString{
            let maxVerticalPipCount = CGFloat(pipsPerFowForRank.reduce(0){max($1.count, 0)})
            let maxHorizontalPipCount = CGFloat(pipsPerFowForRank.reduce(0){max($1.max() ?? 0, 0)})
        ...
        if pipsPerFowForRank.indices.contains(rank){
            let pipsPerRow = pipsPerFowForRank[rank]
